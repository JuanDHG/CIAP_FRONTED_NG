...
import { RolGeneralData, RolSetData, RolStatus, RolPutData,UserData, UserDataRegister,DataProyect, UserDataEdit, UserStatus, DataRolePermison } from "../../../api/datarole.module";
...
export class RolesComponent implements OnInit {
    ...
    valuesCheck(id: any, p: number, ev: boolean): void {
      ...
      const elementoExistenteIndex = this.vectorItems.findIndex(existingItem => existingItem.id_menu === id);
      ...
      if (ev) {
        ...
        if (elementoExistenteIndex === -1) {
          ...
          const elementoExistente = this.vectorItems[elementoExistenteIndex];
        ...
        if (elementoExistenteIndex !== -1) {
          ...
          const elementoExistente = this.vectorItems[elementoExistenteIndex];
          const permisosFiltrados = elementoExistente.id_permisos.filter(permiso => permiso !== p);
    ...
    onSetDataRole() {
        ...
        if (this.setData.nombreRol === null || this.setData.nombreRol === '') {
            this.messageService.add({severity:'error', summary:'El nombre del rol no puede ser vacio'});       
        }
        if (this.vectorItems.length <= 0) {
            this.messageService.add({severity:'error', summary:'El rol debe contar minimo con un permiso'});
        }else{
            this.setData = {
            ...
            this.server.PostSetDataRol(this.setData).subscribe((res)=>{
                ...
                this.messageService.add({severity:'error', summary:'Presentamos Problemas con el backend. Vuelva a intentar en unos minutos....'});
            });
...
// Editar permisos y roles
 ...
 vectorItem: { menus: { id_menu: any; id_permisos: number[] }[] } = { menus: [] };
 id_rol: any; 

addRemovePermissions(id: any, permiso: number, event: boolean): void {
  console.log('Modificar permisos para:', id, 'permiso:', permiso, 'Evento:', event);
    console.log(this.DaoMenu);
  ...
  const elementoExistenteIndex = this.vectorItem.menus.findIndex(menu => menu.id_menu === id);
  ...
  if (elementoExistenteIndex !== -1) {
    ...
    if (event) {
      ...
      // Eliminar permiso
      menuToUpdate.id_permisos = menuToUpdate.id_permisos.filter(p => p !== permiso);
    }
  } else {
  ...
  this.DaoMenu.forEach(item => {
    const menu = this.vectorItem.menus.find(m => m.id_menu === item.id_menu);
...
getMenus(): { menus: { id_menu: any; id_permisos: number[] }[] } {
  const menus: { menus: { id_menu: any; id_permisos: number[] }[] } = { menus: [] };
  ...
  return permisos;
    ...
    SendServer(){
        ...
        if (this.Permisos.menus.length > 0) {
            ...
            this.messageService.add({severity:'error', summary:'Debe moficiar el nombre permisos los permiso para editar'});
        }
    ...
    SendEndPoind(){
        this.server.PutDataRole(this.putDataRole).subscribe((res)=>{
            ...
            this.server.PostPutPermison(this.Permisos).subscribe((res) => {
                console.log(res);
                ...
                console.log(error);
                this.messageService.add({severity:'error', summary:error['error'].mensaje[0]});
            })
